...
//===============  Software Serial Setup and Variables
#define rxPin 4     //used for softwareserial
#define txPin 5     //used for softwareserial
...
//===============  ENCODER Setup and variables    
  ...
  #define encoder0PinB  2
    volatile unsigned int encoder0Pos = 30000;
    int oldpos = 0;
    int PastA = 0;
    int PastB = 0;
...
void READ_DATA()  //    Read data back from the radio
{
    ...
    while (CAT.available() > 0 && newData == false) {
        ...
        if (recvInProgress == true) 
        {
            ...
            Serial.println();   
  numReceived = 0;  //when complete, reset number received to avoid inadvertent reuse of data
}
...
void GET_RADIO_ID()  //Get Radio ID
{
  ...
  Serial.println();
  Serial.print("Address received (and stored) = ");  
  Serial.print(xcvr, HEX);
  Serial.println();
...
//  delay(10);
  ...
  {
    Serial.print(rcvFromRadio[8 - i],HEX);
    Serial.print("-");
    frequency += (rcvFromRadio[8 - i] >> 4) * decMulti[i * 2];        //this should be data spot 9, not 8, but it's losing a preamble byte on readback.  wierd.
    frequency += (rcvFromRadio[8 - i] & 0x0F) * decMulti[i * 2 + 1];
  }
...
/*
 * 
 * Might need to do some additional work on the encoder event - if the time is triggered, then 
 * read freq, increment/decrement, then write back - will that be too slow at 19200?  Might be able 
 * to up the serial speed to 57600.
 * 
 * 
 * 
 * When tuning based on microsecond sampling:
 ...
 * 
 * 
 * HEre's the routines used last time, when this was working:
 * 
 * //PIN's definition
...
void doEncoderA()   // look for a low-to-high on channel A
{
//  GET_FREQ();
...
void doEncoderB() 
{
  ...
  /*
      ...
      Serial.print(oldpos);      
      Serial.print(" - ");
      Serial.print(frequency + oldpos);      
      Serial.println();   
      frequency = frequency + oldpos;
      SEND_FREQ(frequency);
      oldpos = encoder0Pos;
*/
}
...
void setup() {
    ...
    GET_RADIO_ID();   //autodetect the radio
    dummycount=0;
...
/*
 * Encoder setup:
 * need to impliment a speed function within the encoder operation to change tuning step based on knob speed.
 */
